...
import com.insulinhero.serverv5.config.security.JdbcUserDao;
import com.insulinhero.serverv5.config.security.login.User;
import com.insulinhero.serverv5.config.security.login.UserNotFoundException;
...
public class JdbcUserDaoTests extends BaseDaoTests {
    ...
    public void create_user_with_null_username() {
        dao.create(null, USER_3.getPassword(), "ROLE_USER");
    }

    @Test(expected = DataIntegrityViolationException.class)
    public void create_user_with_existing_username() {
        dao.create(USER_1.getUsername(), USER_3.getPassword(), "ROLE_USER");
    }

    @Test(expected = IllegalArgumentException.class)
    public void create_user_with_null_password() {
        dao.create(USER_3.getUsername(), null, "ROLE_USER");
    }
    ...
    public void create_user_creates_a_user() {
        ...
        boolean userWasCreated = dao.create(newUser.getUsername(), newUser.getPassword(), "ROLE_USER");

        Assert.assertTrue(userWasCreated);

        User actualUser = dao.findByUsername(newUser.getUsername());
